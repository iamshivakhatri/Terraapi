        ...
        case "daily":
            if (resource == "APPLE"){
                ContentView.terraManager?.getDaily(type: .APPLE_HEALTH, startDate: DataView.startDate_, endDate: DataView.endDate_, toWebhook: false){success,data,error in
                    ...
                    DataDisplay.dataToDisplay.append(DataField(name: "Steps", value:  (lastDailyPayload?.distance_data?.steps).toString()))
                    DataDisplay.dataToDisplay.append(DataField(name: "Floors Climbed", value:  (lastDailyPayload?.distance_data?.floors_climbed).toString()))
                    ...
                    DataDisplay.dataToDisplay.append(DataField(name: "Total Distance", value:  (lastDailyPayload?.distance_data?.distance_meters).toString() + " m"))
